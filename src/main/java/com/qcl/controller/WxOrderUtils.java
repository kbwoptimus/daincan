...
//    }
    ...
    public void exportOrderToExcel(HttpServletResponse response) throws IOException {
        ...
        String[] titles = {"订单id", "用户姓名", "用户手机号", "送餐地址" ,"店家名", "订单金额", "订单状态", "下单时间"};
        ...
        for (int i = 0; i < size; i++) {
            ...
            dataList[i][3] = orderRoot.getAddress();
            dataList[i][4] = orderRoot.getStname();
            dataList[i][5] = "" + orderRoot.getOrderAmount();
            dataList[i][6] = EnumUtil.getByCode(orderRoot.getOrderStatus(), OrderStatusEnum.class).getMessage();
            dataList[i][7] = "" + orderRoot.getCreateTime();
        }
